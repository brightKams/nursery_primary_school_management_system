...
include("connection.php"); //Establishing connection with our database
    ...
    else
    {
        ...
        $sql="SELECT roleid, Accountstatus,password,password_hash FROM users WHERE username='$username' and password='$password'";

        $result=mysqli_query($db,$sql);

        $row=mysqli_fetch_array($result);
        if(mysqli_num_rows($result) == 1 && $row["roleid"]=="1" && $row["Accountstatus"]=="1" )
        ...
        elseif(mysqli_num_rows($result) == 1 && $row["roleid"]=="2" && $row["Accountstatus"]=="1")
        {
            header("location: profile-update.php"); // Should Redirecting To parentPage.php/UsersPage.php
        } 
        elseif(mysqli_num_rows($result) == 1 && $row["roleid"]=="" && $row["Accountstatus"]=="1")
        {
